...
class LevelView(arcade.View):
    ...
    def __init__(self, position_slovar, new_game):
        ...
        self.keys_for_five_sposob = [arcade.key.NUM_1, arcade.key.RETURN, arcade.key.NUM_3, arcade.key.NUM_2, arcade.key.NUM_0]
    ...
    def center_kamera_za_igrok(self):
        ...
        if self.kamera_dvizh:
            self.kamera.move_to(self.dop_func(ekran_center_x, ekran_center_y), 0.2)
    ...
    def filling_vrag_lists(self):
        ...
        for vrag in self.zhivie_vrag_list:
            if vrag not in self.fizika.sprites:
    ...
    def press_five_sposobs(self, symbol: int):
        i = 0
        for key in self.keys_for_five_sposob:
            if symbol == key:
                self.igrok.action(self.igrok.five_sposobs[i].sposob)
                return
            i += 1
