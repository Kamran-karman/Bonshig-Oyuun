...
from animations.pers_animations import WALK, BEG, IDLE
import sposobs
import arcade
...
class SinhekmAnimations(animations.pers_animations.AnimationsPlus):
    def __init__(self, sprite: simples.SimpleSprite):
        ...
        main_patch = 'resources/Pers_animations/plus/Sinhelm/'
        self.slovar_animation[IDLE][3].load_textures(6, f'{main_patch}idle/', PNG)
        self.slovar_animation[IDLE][1] = 5
        self.slovar_animation[IDLE][2] = 0.05

        self.ukaz_texture = arcade.load_texture_pair(f'{main_patch}sinhelm_ukaz.png')
        self.jump_texture = arcade.load_texture_pair(f'{main_patch}sinhelm_jump.png')
        self.fall_texture = arcade.load_texture_pair(f'{main_patch}sinhelm_fall.png')
        self.landing_texture = arcade.load_texture_pair(f'{main_patch}sinhelm_landing.png')
        self.podgotovka_texture = arcade.load_texture_pair(f'{main_patch}sinhelm_podgotovka.png')
        self.sprite.texture = self.slovar_animation[IDLE][3][0][0]

        self.slovar_animation[WALK][3].load_textures(8, f'{main_patch}walk/', PNG)
        self.slovar_animation[WALK][1] = 7
        self.slovar_animation[WALK][2] = 0.15
        self.dialog_textures.load_textures(3, f'{main_patch}sinhelm_dialog', PNG)
