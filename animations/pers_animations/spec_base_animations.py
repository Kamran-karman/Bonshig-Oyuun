...
from animations.pers_animations import WALK, IDLE
import arcade
...
class BratislavAnimations(pers_animations.BaseAnimations):
    def __init__(self, sprite: simples.SimpleSprite):
        ...
        self.idle_texture = arcade.load_texture_pair(f'{main_patch}idle.png')
        self.slovar_animation[IDLE][3].load_textures(6, f"{self.main_patch}Adnot/idle/", PNG)
...
class OstAnimations(pers_animations.BaseAnimations):
    def __init__(self, sprite: simples.SimpleSprite, back: bool):
        super().__init__(sprite)

    def create_walk_animations(self, main_patch_walk: str):
        self.slovar_animation[WALK][3].load_textures(8, main_patch_walk, PNG)
        self.slovar_animation[WALK][1] = 7
        self.slovar_animation[WALK][2] = 0.2
        self.sprite.texture = self.idle_texture[1]


class Voin0Animations(OstAnimations):
    def __init__(self, sprite: simples.SimpleSprite, back: bool):
        super().__init__(sprite, back)
        main_patch = f"{self.main_patch}male_person/malePerson_"

        self.idle_texture = arcade.load_texture_pair(f'{main_patch}idle.png')
        if not back:
            self.create_walk_animations(f'{main_patch}walk')


class Voin1Animations(OstAnimations):
    def __init__(self, sprite: simples.SimpleSprite, back: bool):
        super().__init__(sprite, back)
        main_patch = f"{self.main_patch}male_adventurer/maleAdventurer_"
...
class Voin2Animations(OstAnimations):
    def __init__(self, sprite: simples.SimpleSprite, back: bool):
        super().__init__(sprite, back)
        main_patch = f"{self.main_patch}female_adventurer/femaleAdventurer_"
...
class Voin3Animations(OstAnimations):
    def __init__(self, sprite: simples.SimpleSprite, back: bool):
        super().__init__(sprite, back)
        main_patch = f"{self.main_patch}female_person/femalePerson_"
