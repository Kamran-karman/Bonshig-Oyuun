...
class Ogon(stihiya.StihiyaFight):
        ...
        for sprite in self.sprite_list:
            for sposob in sprite.tik_slovar:
                if sposob == self.sposob and sprite.tik_slovar[sposob][0]:
                    ...
                    if sprite.tik_slovar[sposob][1] % self.__interval == 0:
    ...
    def check_voda_colision(self, sprite):
        def true_func():
            self.s += self.timer_for_s
            self.change_x = 0
            self.change_y = 0

        for voda in sprite.sposob_list:
            if arcade.check_for_collision(voda, self) and voda.podklass == sposobs.VODA:
                if voda.action:
                    true_func()
                    return True
                elif voda.block_sposob:
                    if voda.block:
                        true_func()
                        return True
        return False
...
class MiniFireBall(Ogon):
    def __init__(self, pers, sprite_list):
        ...
        self.kast = True
        self.timer_for_s_kast = 20
        self.s_kast = 0
        self.timer_for_s = 90 + self.timer_for_s_kast
        self.timer_for_s_kd = 180
        self.s_kd = self.timer_for_s_kd
    ...
    def on_update(self, delta_time: float = 1 / 60) -> None:
        self.update_sposob()
        self.update_mor()
        if self.pers.oglush and self.kast:
            self.kast = False
            self.s_kast = 0
            self.s += self.timer_for_s * 2
            self.change_x = 0
...
class KulakOgnya(Ogon):
    def __init__(self, pers, sprite_list):
        ...
        self.timer_for_s = 15 + self.timer_for_s_kast
        self.timer_for_s_kd = 120
        self.s_kd = self.timer_for_s_kd
        self.timer_for_s_tik = 90
...
class OgnenSnaryad(Ogon):
    def __init__(self, pers, sprite_list):
        ...
        self.kast = False
        self.s_kast = 0
        self.timer_for_s_kast = 12
        self.timer_for_s = 60 + self.timer_for_s_kast
        self.timer_for_s_kd = 120
        self.s_kd = self.timer_for_s_kd
        self.timer_for_s_tik = 120

        self.tex = arcade.load_texture_pair('resources/Sposob_animations/ognen_snaryad.png')
        self.texture = self.tex[1]
        self.storona = 0
    ...
    def update_animation(self, delta_time: float = 1 / 60) -> None:
        if self.action:
            self.texture = self.tex[self.pers.storona]
