...
class MechOgon(ColdOruzhie, ogon.Ogon):
    def __init__(self, pers, sprite_list, timer_for_s, timer_for_s_kd):
        ...
        self.uron = 50
        self.tik_uron = 5
        self.minus_stamina = 30
        self.minus_block_stamina = 5
        self.minus_mana = 1
        ...
        self.ukol_texture = arcade.load_texture_pair('nuzhno/mech_ukol.png')
        self.ukol_ogon_texture = arcade.load_texture_pair('nuzhno/mech_ukol_ogon.png')
        self.scale = 2
    ...
    def update_animation(self, delta_time: float = 1 / 60) -> None:
        self.update_storona()
        if self.action and not self.block:
            if self.ogon_state:
                if self.ukol:
                    self.texture = self.ukol_ogon_texture[self.pers.storona]
                else:
                    self.texture = self.udar_ogon_texture[self.pers.storona]
            elif self.ukol:
                self.texture = self.ukol_texture[self.pers.storona]
...
class Molotok(ColdOruzhie):
    def __init__(self, pers, sprite_list):
        ...
        self.minus_block_stamina = 1

        self.udar_texture = arcade.load_texture_pair("resources/Sposob_animations/molotok.png")
        self.block_texture = arcade.load_texture_pair("resources/Sposob_animations/molotok_block.png")
        self.texture = self.udar_texture[0]

        self.angle = 15

        self.speed_angle = 75 / self.timer_for_s

    def on_update(self, delta_time: float = 1 / 60, physics_engine: arcade.PymunkPhysicsEngine = None) -> None:
        self.center_y = self.pers.center_y - 40
        if self.pers.storona == 0:
            self.center_x = self.pers.center_x + 40
        else:
            self.center_x = self.pers.center_x - 40
        self.radius.position = self.pers.position
