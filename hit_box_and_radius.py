...
class Radius(arcade.BasicSprite):
    def __init__(self, pers, razmer=1):
        """
        :param razmer: 1 = 1000x1000
        """
        super().__init__(arcade.load_texture("nuzhno/radius_porazheniya.png"))
...
class KvadratRadius(Radius):
    def __init__(self, pers, razmer=1):
        ...
        self.texture = arcade.load_texture('resources/kvadrat_radius.png')
        self.hit_box._points = self.texture.hit_box_points

    def update(self) -> None:
        self.scale = self.pers.scale - 0.1
        self.position = self.pers.position
